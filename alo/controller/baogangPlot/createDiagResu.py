...
class createDiagResu:
    ...
    def run(self, width, length, thickness,platetype):
        ...
        '''
        # print(self.upid)
        # wideData = custom_input['wideData']
        ...
        #     'upid': self.upid
        # })
        # plateKindMess['data'] = data
        # mostId = dataInKind().general_call(plateKindMess)['finalResult']


        # colonmName = wideData.columns.values
        # goodBoardId = list(wideData[wideData['upid_flag']==1].index)
        # goodIdInKind = [item for item in goodBoardId if item in mostId]
        # badBoardId = list(wideData[wideData['upid_flag']==0].index)
        # badIdInKind = [item for item in badBoardId if item in mostId]
        # # allBoardData = wideData.loc[mostId].values
        # goodBoardData = wideData.loc[goodIdInKind].values
        # badBoardData = wideData.loc[badIdInKind].values
        # allBoardId = goodIdInKind + badIdInKind
...
# 数据是经过筛选分级获得
        ...
        # goodBoardData = dataRead
        # badBoardData = dataRead
        # allBoardId= dataReadChoose
